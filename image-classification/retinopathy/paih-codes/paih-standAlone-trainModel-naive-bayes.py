...
model_nb.write().overwrite().save(imageDir + 'model-naive-bayes')

predictions = model_nb.transform(featureVector).persist()

#predictionAndLabels = predictions.select("prediction", "label")
evaluator = MulticlassClassificationEvaluator(metricName="accuracy")
print("Train Data set accuracy with Naive Bayes = " + str(evaluator.evaluate(predictions)) + " and error " + str(1 - evaluator.evaluate
(predictions)))
